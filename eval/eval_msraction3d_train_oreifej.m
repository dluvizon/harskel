function eval_msraction3d_train_oreifej(id, idLog, splits, opts)
...
for ID = id(:)
    ...
    for SubTest = splits
        [par.listTr, par.listTe] =...
            select_subject_splitting(par.allSamples, TrSubs(SubTest,:));
        ...
        logName = sprintf('%s/log_%s_%04d_%04d_%04d',...
            outdir.log, par.datasetName, ID, idLog, SubTest);
        savevar(logName, [accr0 accr1 accr2 bestAcc]);
    end

    fprintf('Accuracy FINAL: %.1f%%  --> %.1f%%  (best = %.1f%%) :: std: %g %g %g\n',...
